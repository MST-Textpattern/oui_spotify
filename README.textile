...
"oui_spotify":https://github.com/NicolasGraph/oui_spotify is a "oui_player":https://github.com/NicolasGraph/oui_player v2+ extension to easily embed "Spotify":https://www.spotify.com/ provided videos in "Textpattern":https://textpattern.com/ powered websites.
...
oui_spotify's minimum requirements:
...
# "Download":https://github.com/NicolasGraph/oui_spotify/releases the compiled plugin file or the source to compile a customized file.
...
# Install oui_spotify:
@$ composer require nicolasgraph/oui_spotify@
...
bc. $ composer update nicolasgraph/oui_spotify
...
bc. $ composer remove nicolasgraph/oui_spotify

h2. Tags and parameters

h3. oui_spotify vs oui_player

While the @<txp:oui_player />@ tag is able to embed any video from any extension-related provider, the @<txp:oui_spotify />@ tag is a shortcut to embed Spotify videos only. It is strictly equivalent to @<txp:oui_player provider="spotify" />@.
...
h3. oui_if_spotify vs oui_if_player

p. While the @<txp:oui_if_player>…</txp:oui_if_player>@ tag is able to check an URL against any URL from any extension-related provider, the @<txp:oui_if_spotify>…</txp:oui_if_spotify>@ tag is a shortcut to check an URL against Spotify related URL schemes only. It is strictly equivalent to @<txp:oui_if_player provider="spotify">…</txp:oui_if_player>@.
...
"oui_player":https://github.com/NicolasGraph/oui_player preference related field value: @https://open.spotify.com/album/4RuzGKLG99XctuBMBkFFOC@.

h3. Embed a player by defining its provider and a media ID through attributes

bc(language-markup). <txp:oui_player provider="spotify" play="album/4RuzGKLG99XctuBMBkFFOC" />

…or:

bc(language-markup). <txp:oui_spotify play="album/4RuzGKLG99XctuBMBkFFOC" />

h3. Conditionally embed a player using attributes

bc(language-markup).. <txp:oui_if_player provider="spotify" play="https://open.spotify.com/track/4vtgqkR7eeosgF0h4QD4kQ">
    <txp:oui_player />
</txp:oui_if_player>

p. …or:

bc(language-markup).. <txp:oui_if_spotify play="https://open.spotify.com/track/4vtgqkR7eeosgF0h4QD4kQ">
    <txp:oui_spotify />
</txp:oui_if_spotify>
...
oui_spotify version 1.0.0-BETA1, Copyright (C) 2018 Nicolas Morand
...
* *oui_spotify v1.0.0-BETA1* (2018-08-15)
